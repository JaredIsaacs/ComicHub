...
class Page(DatabaseRow):
    ...
    """
    def __init__(self, cursor: Cursor, chapter_id: int, order: int,
                page_url: str, obj_id: int | None = None):
    ...
    def create(self):
        self.cursor.execute('INSERT INTO Pages (chapter_id, order, page_url) VALUES (?, ?, ?)',
                            (self.chapter_id, self.order, self.page_url))
        self.obj_id = self.cursor.lastrowid


    def update(self):
        self.cursor.execute('UPDATE Pages SET chapter_id = ?, order = ?, page_url = ? WHERE id = ?',
                            (self.chapter_id, self.order, self.page_url, self.obj_id))
